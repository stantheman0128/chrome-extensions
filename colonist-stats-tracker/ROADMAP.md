...
(以下是 1.88 之前的狀態與設計方向,已被取代,只當歷史看。)當初 ⛔ 是用「實收對比」:遊戲中很準,但「剛蓋好馬上被擋」那種情形會輕微高估,只有局末讀 Victory 畫面才完全精準。Stan 當時要的是**遊戲進行中每個數值也都正確**——這個需求已由 1.88/1.90/1.91 的幾何時間線達成。
...
- **基礎:** board.js 已能算「強盜壓住某號碼地時,相鄰建築各損失幾張」(`blockedLossOf`),當時只能對「當下」算、還不能重播歷史。要補的「把每回合的地圖+強盜凍結成時間線」這層,後來在 1.88/1.90/1.91 做掉了。
