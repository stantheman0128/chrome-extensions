...
test('roll strip shows the last rolls in order, hidden when empty', () => {
  ...
  cst.createPanel();
  cst.render();
  assert.equal(document.querySelector('[title*="rolls (oldest"]'), null,
    'no strip before the first roll');

  cst.state.rollHistory = [5, 7, 9];
  cst.render();
  const strip = document.querySelector('[title="Last 3 rolls (oldest → newest)"]');
...
test('roll strip caps at the last 12 rolls', () => {
  ...
  cst.render();
  const strip = document.querySelector('[title="Last 12 rolls (oldest → newest)"]');
