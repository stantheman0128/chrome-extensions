...
test('aggregate counts games / wins / income / turn time / steals over played games', () => {
  const a = popup.aggregate([
    game({ winner: 'Me',  tally: { Me: { gained: 30, stole: 2, lost: 1, turnMs: 100000, turns: 10 } } }),
    game({ winner: 'Bob', tally: { Me: { gained: 20, stole: 0, lost: 3, turnMs: 80000,  turns: 8  } } }),
    game({ winner: 'Me',  tally: { Me: { gained: 40, stole: 4, lost: 0, turnMs: 120000, turns: 12 } } }),
  ]);
  ...
  assert.equal(a.avgSteals, 2, 'avg steals = (2+0+4)/3');
